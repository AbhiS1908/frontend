...
import SegregationStockDetails from './SegregationStockDetails';

const SegregationMasterCash = () => {
  ...
  useEffect(() => {
    if (!cashId) return; // Prevent API call if cashId is not available

    const fetchData = async () => {
      try {
        const response = await axios.get(`http://localhost:8000/api/v1/auth/cash-form/${cashId}`);
    ...
    fetchData();
  }, [cashId]); // Added cashId as a dependency
                ...
                <tr>
                  <td colSpan="7">
                    {expandedUnitType === 'bag' ? (
                      <SegregationStockDetails stockData={stockData[item._id]} />
                    ) : null}
                  </td>
                </tr>
              )}
...
export default SegregationMasterCash;
