...
const BulkCashTauli = () => {
    ...
    const cashStockId = searchParams.get('cashFormId'); // Extract cashFormId from URL

    const [formData, setFormData] = useState({
        cashStockId: cashStockId || '', // Pre-fill cashStockId from URL
        makhana: '',
        quantity: 0,
        bagCount: 0,
    ...
    // Fetch all records on component mount or when cashStockId changes
    useEffect(() => {
        if (cashStockId) {
            fetchRecords();
        }
    }, [cashStockId]); // Only re-run if cashStockId changes

    // Fetch all records for the given cashStockId
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setFormData({
                cashStockId: cashStockId || '', // Reset form with cashStockId from URL
                makhana: '',
                quantity: 0,
                bagCount: 0,
    ...
    return (
        <Layout>
            ...
            <h1 style={styles.heading}>Bulk Cash Tauli</h1>

            {/* Form for creating/updating records */}
            <form onSubmit={handleSubmit} style={styles.form}>
                <div style={styles.formGroup}>
                    <label style={styles.label}>CashStock ID:</label>
                    <input
                        type="text"
                        name="cashStockId"
                        value={formData.cashStockId}
            ...
            <table style={styles.recordsTable}>
                <thead>
                    <tr>
                        <th style={styles.tableHeader}>CashStock ID</th>
                ...
                <tbody>
                    {records.map((record) => (
                        <tr key={record._id} style={styles.tableRow}>
                            <td style={styles.tableCell}>{record.cashStockId}</td>
