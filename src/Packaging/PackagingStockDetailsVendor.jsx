...
const PackagingStockDetailsVendor = ({ row, _id }) => {
    const [stockDetails, setStockDetails] = useState(null);

    useEffect(() => {
        const fetchStockDetails = async () => {
          try {
            const response = await fetch(`https://ane-production.up.railway.app/api/v1/auth/vendor-stock-one/${_id}`);
            if (!response.ok) {
      ...
      return (
        <div style={modalContentStyle}>
          <h2 style={modalHeaderStyle}>Details for {row.vendorName || row.name}</h2>
          <div style={gridContainerStyle}>
            <div style={gridStyle}>
              {/* Always displayed fields */}
              <div style={fieldStyle}>
                <strong>Name:</strong> {stockDetails.vendorName || stockDetails.name}
              </div>
...
export default PackagingStockDetailsVendor;
