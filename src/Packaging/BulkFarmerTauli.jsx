...
const BulkFarmerTauli = () => {
    const [searchParams] = useSearchParams();
    const farmerStockId = searchParams.get('farmerFormId'); // Extract farmerFormId from URL

    const [formData, setFormData] = useState({
        farmerStockId: farmerStockId || '', // Pre-fill farmerStockId from URL
        makhana: '',
        quantity: 0,
        bagCount: 0,
    ...
    // Fetch all records on component mount or when farmerStockId changes
    useEffect(() => {
        if (farmerStockId) {
            fetchRecords();
        }
    }, [farmerStockId]); // Only re-run if farmerStockId changes

    // Fetch all records for the given farmerStockId
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setFormData({
                farmerStockId: farmerStockId || '', // Reset form with farmerStockId from URL
                makhana: '',
                quantity: 0,
                bagCount: 0,
    ...
    return (
        <Layout>
            ...
            <h1 style={styles.heading}>Bulk farmer Tauli</h1>

            {/* Form for creating/updating records */}
            <form onSubmit={handleSubmit} style={styles.form}>
                <div style={styles.formGroup}>
                    <label style={styles.label}>farmerStock ID:</label>
                    <input
                        type="text"
                        name="farmerStockId"
                        value={formData.farmerStockId}
            ...
            <table style={styles.recordsTable}>
                <thead>
                    <tr>
                        <th style={styles.tableHeader}>farmerStock ID</th>
                ...
                <tbody>
                    {records.map((record) => (
                        <tr key={record._id} style={styles.tableRow}>
                            <td style={styles.tableCell}>{record.farmerStockId}</td>
...
export default BulkFarmerTauli;