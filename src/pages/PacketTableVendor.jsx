...
const PacketTableVendor = () => {
  ...
  const [disabledProductRows, setDisabledProductRows] = useState({}); // Track disabled product rows
  ...
  const vendorStockId = params.get("vendorFormId");
  ...
  const fetchProductEntries = async (vendorStockDetailsId) => {
    try {
      ...
      setProductEntries((prev) => ({
        ...prev,
        [vendorStockDetailsId]: response.data,
      }));
    } catch (error) {
  ...
  const handleProductEntrySave = async (vendorStockDetailsId, index) => {
    try {
      const payload = {
      ...
      setDisabledProductRows((prev) => ({
        ...prev,
        [`${vendorStockDetailsId}-${index}`]: true,
      }));
  ...
  const handleAddProductClick = (index, vendorStockDetailsId) => {
...
export default PacketTableVendor;