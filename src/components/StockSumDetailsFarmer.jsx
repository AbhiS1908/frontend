...
import EditComponentPLSDFarmer from '../Edit-delete/EditComponentPLSDFarmer';

const StockSumDetailsFarmer = ({ stockData,  farmerFormId, fetchStockData }) => {
    const [loading, setLoading] = useState(false);
    const [editStock, setEditStock] = useState(null);
    const navigate = useNavigate();

    const handleExpand = (stock) => {
      const farmerFormId = stock._id;
      const stockObject = encodeURIComponent(JSON.stringify(stock));

      if (farmerFormId) {
          navigate(`/detail-log-per-bag-farmer?farmerFormId=${farmerFormId}&stockData=${stockObject}`);
      } else {
          console.error("farmerFormId is missing");
      }
  };
  ...
  const handleDeleteClick = async (stockId) => {
    ...
    setLoading(true);
    try {
      await axios.delete(`https://ane-production.up.railway.app/api/v1/auth/farmer-stock/${stockId}`);
      alert("Stock deleted successfully");
      await fetchStockData(farmerFormId); // Refresh data after delete
    } catch (error) {
  ...
  const handleUpdateStock = async () => {
    await fetchStockData(farmerFormId); // Refresh data after update
    setEditStock(null);
...
export default StockSumDetailsFarmer;
