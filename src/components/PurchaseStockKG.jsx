...
const PurchaseStockKG = ({ cashFormId }) => {
  ...
  const handleSave = async (row, index) => {
    ...
    const payload = {
      cashFormId: cashFormId,
      cashname: "", // Not taking a post value
      hsn: "080290", // Hardcoded as per your requirement
      quantity: row.quantity || 0,
      product: row.product,
      rate: row.pricePerKg || 0, // Use pricePerKg as rate
      pricePerKgBag: pricePerKg, // Ensure pricePerKgBag is included
      gst: row.gst || 0, // Include GST if needed
      amount: totalCost,
...
export default PurchaseStockKG;