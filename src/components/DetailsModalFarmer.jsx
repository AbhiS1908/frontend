...
const DetailsModalFarmer = ({ item, onClose, onSave }) => {
  ...
  const handleSaveClick = async () => {
    try {
      await axios.put(`http://localhost:8000/api/v1/auth/farmer-form/${item._id}`, formData);
  ...
  const confirmDelete = async () => {
    try {
      await axios.delete(`http://localhost:8000/api/v1/auth/farmer-form/${item._id}`);
...
export default DetailsModalFarmer;
